...
void TutorialApp::ModelRender()
{
	...
	for (size_t i = 0; i < m_pModel->m_Meshes.size(); i++)
	{
		...
		m_pDeviceContext->PSSetShaderResources(5, 1, &m_pModel->m_Materials[mi].m_pMetalnessRV);
		m_pDeviceContext->PSSetShaderResources(6, 1, &m_pModel->m_Materials[mi].m_pRoughnessRV);
