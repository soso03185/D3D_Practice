...
float4 main(PS_INPUT Input) : SV_Target
{
    ...
    float3 Emissive = 0;
    if (UseEmissiveMap) Emissive = txEmissive.Sample(samLinear, Input.Tex);
    
    float Opacity = 1.0f;
    if (UseOpacityMap)  Opacity = txOpacity.Sample(samLinear, Input.Tex).a;
    
    float3 ambient = gCubeAmbient * texColor;
    float4 finalColor = float4(ambient + diffuse + specular + Emissive, Opacity) * vLightColor;

    if (UseGamma) finalColor.rgb = pow(finalColor.rgb, 1 / 2.2f); // gamma
    return finalColor;
};
